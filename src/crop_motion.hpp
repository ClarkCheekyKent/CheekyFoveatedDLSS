...
// Select the pixel units of the supplied scale, independently of the texture grid.
// Native NGX selects input or output pixel units according to its MV flag.
// Streamline supplies normalized scale; callers multiply by the selected view size.
inline bool crop_motion_offset(const FoveationGeometry& previous,
    const FoveationGeometry& current, bool input_pixel_units, float scale_x,
    ...
    const auto dx = static_cast<double>(input_pixel_units ? current.input_base_x : current.output_base_x) -
        static_cast<double>(input_pixel_units ? previous.input_base_x : previous.output_base_x);
    const auto dy = static_cast<double>(input_pixel_units ? current.input_base_y : current.output_base_y) -
        static_cast<double>(input_pixel_units ? previous.input_base_y : previous.output_base_y);
...
// Optional output dimensions point-resample the grid and scale displacement.
// Normalized-vector callers must compensate their scale by source/output size.
struct CropMotion11;
std::shared_ptr<CropMotion11> create_crop_motion11(ID3D11DeviceContext*,
    ...
    CropMotionOffset, unsigned output_width = 0U, unsigned output_height = 0U) noexcept;
