...
1. Close the game and install the complete `afw-auto-1` Cheeky UEVR ZIP using the folder layout above, replacing both DLLs and the Lua script. Keep your existing AFW UEVR build. Start with the game's native DLSS Super Resolution path, without an upscaler replacement/proxy.
