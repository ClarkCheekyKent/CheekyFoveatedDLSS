...
This integration targets **DX12 with the public, unmodified AFW UEVR build**. It does not add AFW to mainline UEVR or require a custom UEVR executable. The SR implementation includes fixed and gaze-driven coverage, center supersampling, history recovery, controls and diagnostics. The original centered mode kept AFW enabled in a Hogwarts Legacy user test. The completed integration still needs in-game testing; automated tests simulate the hook chain, not NVIDIA inference or the warp shader.
...
1. Close the game and install the complete `afw-sr-1` Cheeky UEVR ZIP using the folder layout above, replacing both DLLs and the Lua script. Keep your existing AFW UEVR build. Start with the game's native DLSS Super Resolution path, without an upscaler replacement/proxy.
