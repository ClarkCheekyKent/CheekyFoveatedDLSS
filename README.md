...
4. UEVR AFW has an experimental DX12 routing path with centered, manual or automatic stereo coverage, center supersampling, and warp eye/mode diagnostics on verified builds. Gaze, marker calibration and NR remain bypassed. The original centered mode kept AFW enabled in a Hogwarts Legacy user test; the additions need in-game validation. [AFW test instructions](uevr/README.md#afw-routing-experiment)
