...
void note_d3d12_present(ID3D12CommandQueue*) noexcept { collect_dlss_nr_input_submissions(); collect_crop_motion12(); }
...
int main() {
    ...
    const auto motion = run_crop_motion_lifetime_tests();
    return motion ? motion : run_d3d12_composite_tests();
}
