...
void run(float pre,float scale,float exposure_value,float expected,bool normalize=true,DXGI_FORMAT output_format=DXGI_FORMAT_R32G32B32A32_FLOAT) {
 ...
 D3D12_ROOT_PARAMETER params[3]{};params[0].ParameterType=params[1].ParameterType=D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;params[0].DescriptorTable={2,srvs};params[1].DescriptorTable={1,&ur};params[2].ParameterType=D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;params[2].Constants={0,0,48};
 ...
 ID3D12DescriptorHeap* heaps[]{heap.Get()};l->SetDescriptorHeaps(1,heaps);l->SetComputeRootSignature(root.Get());l->SetPipelineState(pipeline.Get());auto gpu=heap->GetGPUDescriptorHandleForHeapStart();l->SetComputeRootDescriptorTable(0,gpu);gpu.ptr+=2*stride;l->SetComputeRootDescriptorTable(1,gpu);l->SetComputeRoot32BitConstants(2,48,&c,0);l->Dispatch(8,8,1);
